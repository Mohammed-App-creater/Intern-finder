...
import {
  useTalentDashboardTotalInterviews,
  useTalentDashboardTotalJobsApplied,
} from "@/hooks/useTalentDashboard";
...
export function StatusCards() {
  ...
  const {
    ...
    isError: JobLoading,
  } = useTalentDashboardTotalJobsApplied(user?.id ?? "");
  const { data, isLoading, isError } = useTalentDashboardTotalInterviews(
    user?.id ?? ""
  );
