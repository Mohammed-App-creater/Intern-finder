...
import { useTalentDashboardStatus, useTalentDashboardTotalJobsApplied } from "@/hooks/useTalentDashboard";
...
export function StatusCards() {
  ...
  const { data, isLoading, isError } = useTalentDashboardStatus(user?.id ?? "");
        ...
        <CardContent className="relative py-3 px-10">
          <div className="flex items-center justify-between">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-5xl font-bold text-dark">{data?.jobStatusCounts.interviewed}</p>
            </div>
            <Image
              src={InterviewIcon}
              alt={"Document Icon"}
