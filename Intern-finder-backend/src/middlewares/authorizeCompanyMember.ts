...
export const authorizeCompanyMember = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (user.id === companyId && user.role === 'COMPANY') return next();
  if (user.companyId && user.companyId === companyId && user.companyMember) return next();
...
export const authorizeCompanyAdmin = (req: Request, res: Response, next: NextFunction) => {
  console.log("authorizeCompanyAdmin middleware invoked", req.user, req.params);
  const user = req.user as any;
  const { companyId } = req.params;
  if (!user) return res.status(401).json({ error: "Unauthorized" });
